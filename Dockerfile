...
RUN apt-get update \
    ...
    && rm -rf /var/lib/apt/lists/* \
    && installUtility install --acceptLicense apmDataCollector-7.4 && installUtility install --acceptLicense defaultServer || if [ $? -ne 22 ]; then exit $?; fi \
    && rm -rf /opt/ibm/wlp/output/defaultServer/workarea
