...
+ Get the application URL by running these commands:
  ...
  {{- end }}
  {{- else }}
  export INGRESS_IP=$(kubectl get ing --namespace {{ $root.Release.Namespace }} {{ include "slt.utils.fullname" (list $root) }} -o jsonpath="{.status.loadBalancer.ingress[0].ip}")
  export APP_PATH={{ $root.Values.ingress.path }}
  {{- if $root.Values.ssl.enabled }}
  echo https://$INGRESS_IP$APP_PATH
  {{- else }}
  echo http://$INGRESS_IP$APP_PATH
  {{- end }}
  {{- end -}}
